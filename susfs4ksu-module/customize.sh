...
if [ ${ver} -lt 5 ]; then
	...
	chmod +x "${TMPDIR}/susfs/tools/latest/${KERNEL_VERSION}/ksu_susfs${ARCH_BIN}"
	if [ ${ARCH} = "arm64" ] || [ ${ARCH} = "x86_64" ]; then
		SUSFS_VERSION_RAW="$(${TMPDIR}/susfs/tools/latest/${KERNEL_VERSION}/ksu_susfs${ARCH_BIN} show version)"
...
if [ -n "$SUSFS_VERSION_RAW" ] && [ "$SUSFS_DECIMAL" -gt 152 ] 2>/dev/null; then
	...
	if download "https://raw.githubusercontent.com/sidex15/susfs4ksu-binaries/main/$SUSFS_DECIMAL/$KERNEL_VERSION/ksu_susfs${ARCH_BIN}" > ${MODPATH}/ksu_susfs_remote ; then
		...
		if ${MODPATH}/ksu_susfs_remote > /dev/null 2>&1 ; then
			...
			# test failed
			cp ${TMPDIR}/susfs/tools/latest/${KERNEL_VERSION}/ksu_susfs${ARCH_BIN} ${DEST_BIN_DIR}/ksu_susfs
		fi
	else
		# failed
	...
	cp -f ${TMPDIR}/susfs/tools/152/${KERNEL_VERSION}/ksu_susfs${ARCH_BIN} ${DEST_BIN_DIR}/ksu_susfs
fi
...
cp ${TMPDIR}/susfs/tools/sus_su${ARCH_BIN} ${DEST_BIN_DIR}/sus_su
